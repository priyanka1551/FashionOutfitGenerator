...
class TextToImageView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            user_history = UserHistory.objects.create(user=user, description=new_prompt, blob_url=blob_url)
            user_history.save()
            response_data = {
                "user_id": user_id,
                "id": user_history.id,
            ...
            print(e)
            return Response("Error... Failed Tasks ", status=status.HTTP_400_BAD_REQUEST)
